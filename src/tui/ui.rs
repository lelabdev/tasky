...
fn draw_loading(f: &mut Frame, size: Rect) {
    let text = vec![Line::from("Loading issues..."), Line::from(""), Line::from("Press q to quit")];
...
fn draw_list(f: &mut Frame, app: &App, size: Rect) {
    ...
    let items: Vec<ListItem> = issues
        ...
        .map(|(i, issue)| {
            ...
            let line = if labels.is_empty() {
                Line::from(Span::styled(format!("#{}  {}", issue.number, issue.title), style))
            } else {
                Line::from(vec![
    ...
    let list = List::new(items)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title(" Issues (j/k: navigate, Enter: view, q: quit) "),
        )
        .highlight_style(
            Style::default()
                .bg(Color::DarkGray)
...
fn draw_detail(f: &mut Frame, app: &App, size: Rect) {
    ...
    let footer = Paragraph::new(Line::from(Span::styled(
        "Enter: work on this  |  Esc: back to list  |  j/k: scroll",
